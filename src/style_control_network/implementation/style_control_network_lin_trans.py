...
class LinTransHyperNetwork(nn.Module):
    def __init__(
        self,
        ...
        """
        Ein Hypernetwork, das basierend auf Text-Abstracts Skalierungs- und Verschiebungsvektoren 
        für eine lineare Transformation in einem LLM generiert.

        Args:
            steering_dim: Dimension der generierten Transformationsvektoren (entspricht hidden_size des LLMs)
            target_layers: Liste der Layer-Indizes, die gesteuert werden sollen
        ...
        # Lineare Transformation: Scale (Multiplikation) und Shift (Addition)
        scale_proj = nn.Linear(hidden_dim, steering_dim)
        shift_proj = nn.Linear(hidden_dim, steering_dim)

        style_control_network_modules = {
            'layer_depth_encoder': layer_depth_encoder,
            'mixer': mixer,
            'core': nn.Sequential(mlp1, mlp2, mlp3),
            'scale_proj': scale_proj,
            'shift_proj': shift_proj
        }

        if embedder_name == "AIDA-UPM/star":
            style_control_network_modules['star_proj'] = star_proj
    ...
    def _init_weights(self):
        ...
        # Init Scale auf 0 (Multiplikation)
        nn.init.zeros_(self.style_control_network_mlp['scale_proj'].weight)
        if self.style_control_network_mlp['scale_proj'].bias is not None:
            nn.init.zeros_(self.style_control_network_mlp['scale_proj'].bias)

        # Init Shift auf 0 (Addition)
        nn.init.zeros_(self.style_control_network_mlp['shift_proj'].weight)
        if self.style_control_network_mlp['shift_proj'].bias is not None:
            nn.init.zeros_(self.style_control_network_mlp['shift_proj'].bias)
    ...
    def forward(self, author_abstracts: List[List[str]]) -> Tuple[Dict[int, torch.Tensor], Dict[int, torch.Tensor]]:
        """
        Forward Pass zur Berechnung der Scale- und Shift-Vektoren.
        ...
        Returns:
            Ein Tupel aus zwei Dictionaries (scale_dict, shift_dict), 
            die die Target-Layer auf die zugehörigen Vektoren abbilden.
        """
        ...
        # 4. Generiere nun INDIVIDUELLE Transformations-Vektoren pro Target Layer
        scale_dict = {}
        shift_dict = {}
        ...
        for layer_idx in self.target_layers:
            ...
            # d) Ab durch das Core-MLP auf Transformations-Dimensionen hochprojizieren
            core_out = self.style_control_network_mlp['core'](mixed_latent)

            scale_vector = self.style_control_network_mlp['scale_proj'](core_out)
            shift_vector = self.style_control_network_mlp['shift_proj'](core_out)

            # Pack in Dicts
            scale_dict[layer_idx] = scale_vector
            shift_dict[layer_idx] = shift_vector

        return scale_dict, shift_dict
