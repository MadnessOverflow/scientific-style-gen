...
def run_setup_evaluation(
    ...
    # 3. Modelle für Generierung und Perplexity laden (Nur EINMAL laden)
    print("Lade Modelle...")
    ...
    # 5. Iteration über die verschiedenen Setups
    for setup in setups:
        setup_name = setup["name"]
        steering_vectors_path = setup["path"]
        target_layer_indices = setup["layers"]

        print("\n" + "="*60)
        print(f"STARTE RUN FÜR SETUP: {setup_name} (Lambda={alpha})")
        print("="*60)

        start_time = time.time()

        # Steering Vectors für das aktuelle Setup laden
        ...
        for row in tqdm(val_dataset, desc=f"Generiere ({setup_name})"):
            ...
            active_hooks = []
            # Nur hooken, wenn wir Vektoren für alle Target-Layer dieses Papers gefunden haben
            if len(paper_layer_vectors) == len(target_layer_indices):
                for layer_idx in target_layer_indices:
            ...
            true_authors.append(row["author"])
            paper_ids.append(paper_id)

        # --- EVALUIERUNG ---
        print(f"Evaluiere {setup_name}...")
        ...
        print(
            f"Ergebnis {setup_name}: Style Acc = {style_accuracy:.2%}, Avg PPL = {avg_ppl:.2f} (Dauer: {duration/60:.1f} min)")
    ...
    print("\nEvaluation beendet. Speichere Ergebnisse...")
    os.makedirs(f"{DATA_DIR}/steering_vectors/setup_evaluation", exist_ok=True)
    df_results = pd.DataFrame(tuning_results)
    df_results.to_csv(
        f"{DATA_DIR}/steering_vectors/setup_evaluation/setup_comparison_results.csv", index=False)
    print("Ergebnisse gespeichert unter: data/steering_vectors/setup_evaluation/setup_comparison_results.csv")
    print(df_results)
...
def visualize_results(data_path: str):
    ...
    # 2. Den Plot aufbauen (Balkendiagramme)
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 12))

    setup_names = df['setup_name'].tolist()
    x = np.arange(len(setup_names))
    width = 0.35  # Breite der Balken für Subplot 2

    # --- Subplot 1: Style Accuracy (Balkendiagramm) ---
    bars1 = ax1.bar(x, df['style_accuracy'],
                    color='royalblue', edgecolor='black', zorder=3)
    ax1.set_ylabel('Style Accuracy', fontsize=22)
    ax1.set_xticks(x, labels=setup_names, fontsize=16)
    ax1.tick_params(axis='y', labelsize=14)
    # ax1.set_xticklabels(setup_names, rotation=15, ha='right')
    ax1.grid(True, axis='y', linestyle=':', alpha=0.6, zorder=0)

    # --- Subplot 2: Cross Entropy & Perplexity (Gruppierte Balken) ---
    bars2 = ax2.bar(x - width/2, df['cross_entropy'], width,
                    label='Cross Entropy', color='darkorange', edgecolor='black', zorder=3)
    bars3 = ax2.bar(x + width/2, df['avg_perplexity'], width,
                    label='Avg Perplexity', color='forestgreen', edgecolor='black', zorder=3)

    ax2.set_ylabel('Loss / Perplexity', fontsize=22)
    ax2.set_xticks(x, labels=setup_names, fontsize=16)
    ax2.tick_params(axis='y', labelsize=14)
    # ax2.set_xticklabels(setup_names, rotation=15, ha='right')
    ...
    plt.savefig(
        f'{DATA_DIR}/steering_vectors/setup_evaluation/setup_comparison_results.png', dpi=300)
    print("Plot gespeichert unter: data/steering_vectors/setup_evaluation/setup_comparison_results.png")
    # plt.show()


if __name__ == "__main__":

    # Hier definierst du deine 4 Setups: Name (für den Plot), Dateipfad und die zugehörigen Layer
    SETUPS_TO_TEST = [
        {
            "name": "Own (all layers)",
            "path": f"{DATA_DIR}/steering_vectors/val_contrastive_own_0_1_2_3_4_5_6_7_8_9_10_11_12_13_14_15_16_17_18_19_20_21_22_23_24_25_26_27_28_29_30_31_32_33_34_35.safetensors",
            "layers": list(range(36))
        },
        {
            "name": "Own (18,19,20)",
            "path": f"{DATA_DIR}/steering_vectors/val_contrastive_own_18_19_20.safetensors",
            "layers": [18, 19, 20]
        },
        {
            "name": "Own (20,21,22)",
            "path": f"{DATA_DIR}/steering_vectors/val_contrastive_own_20_21_22.safetensors",
            "layers": [20, 21, 22]
        },
        {
            "name": "Paper (18,19,20)",
            "path": f"{DATA_DIR}/steering_vectors/val_contrastive_paper_18_19_20.safetensors",
            "layers": [18, 19, 20]
        },
        {
            "name": "Paper (20,21,22)",
            "path": f"{DATA_DIR}/steering_vectors/val_contrastive_paper_20_21_22.safetensors",
            "layers": [20, 21, 22]
        }
    ]

    # === 1. Evaluation ausführen ===
    run_setup_evaluation(
        setups=SETUPS_TO_TEST,
    ...
    visualize_results(
        f"{DATA_DIR}/steering_vectors/setup_evaluation/setup_comparison_results.csv")
